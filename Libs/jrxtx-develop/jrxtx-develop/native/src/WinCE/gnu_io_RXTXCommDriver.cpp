...
JNIEXPORT jboolean JNICALL Java_gnu_io_RXTXCommDriver_registerKnownPorts(
		JNIEnv *env, jobject jobj, jint portType) {
	enum {
		PORT_TYPE_SERIAL = 1,
		PORT_TYPE_PARALLEL,
		PORT_TYPE_I2C,
...
JNIEXPORT jboolean JNICALL Java_gnu_io_RXTXCommDriver_isPortPrefixValid(
		JNIEnv *env, jobject jobj, jstring dev) {
	jboolean retVal;
 ...
 */
JNIEXPORT jboolean JNICALL Java_gnu_io_RXTXCommDriver_testRead(JNIEnv *env,
		jobject jobj, jstring dev, jint type) {
	jboolean retVal;
	DWORD dwError;
	...
	if (type == PORT_SERIAL) {
		...
		if (hPort == INVALID_HANDLE_VALUE) { // Could not open the port.
			...
			retVal = JNI_FALSE;
		} else { // Port open OK - let's close it and return TRUE
